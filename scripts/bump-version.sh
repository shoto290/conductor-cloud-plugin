...
# and package-lock.json — and Cursor, the marketplace import, and npm each read
# a different one. Bumping them by hand is how they drifted apart before, so
# this script is the only supported way to move the version.
set -euo pipefail
...
# manifest and package.json, under "metadata" in the marketplace listing, which
# describes this same folder.
read_version() {
